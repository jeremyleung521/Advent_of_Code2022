...
def read_input(file_name):
    ...
    return list_of_lists, curr_occupied, n_elves, directions, consideration, order


def propagate(curr_occupied, directions, consideration, order):
    ...
    for coord in curr_occupied:
        ...
        # check_c = [(coord[0]+dx, coord[1]+dy) for (dx, dy) in directions]
        check_bool = [False for _ in range(8)]
        for idx, direction in enumerate(directions):
            if tuple(map(add, coord, direction)) in curr_occupied:
                check_bool[idx] = True

        if not any(check_bool):  # If no neighbours, just stay in place
            # print('error where')
            count += 1
            new_location[coord] = coord
        else:
            for check_list, output in zip(consideration, order):
                check_check = [check_bool[i] for i in check_list]
                if not any(check_check):
    ...
    curr_occupied = set(new_location.values())
...
def main():
    ## Part 1
    # initialize, curr_occupied, n_elves, directions, conditions, order = read_input("Day23_smaller_input.txt")
    # initialize, curr_occupied, n_elves, directions, conditions, order = read_input("Day23_test_input.txt")
    initialize, curr_occupied, n_elves, directions, conditions, order = read_input("Day23_input.txt")

    for _ in trange(10):
        new_location, curr_occupied, flag = propagate(curr_occupied, directions, conditions, order)
...
def main2():
    ...
    for idx in trange(100000):
        new_location, curr_occupied, flag = propagate(curr_occupied, directions, conditions, order)
        # answer = find_minmax(curr_occupied, n_elves, visualize=True)
        if flag is True:
            print(idx+1)
            answer2 = idx+1
            break
